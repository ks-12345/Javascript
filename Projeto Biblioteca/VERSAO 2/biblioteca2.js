...
function adicionarLivro() {
  ...
  if (titulo && autor && ano) {
    ...
    function buscarLivro(){
        ...
        const resultado = biblioteca.filter((livro) => livro.titulo.toLowerCase().includes(busca));
...
function alterarLivro() {
  if (livroParaAlterar) {
    ...
    function emprestimo() {
      ...
      if (titulo1) {
        alert(`livro ${titulo1}, Emprestado com sucesso!`);
      } else {
    ...
    function atualizarLista(lista= biblioteca) {
        const tabela = document.getElementById("lista-livro");
        tabela.innerHTML= "";
